...
static void IRAM_ATTR jhs_ac_rx_isr(QueueHandle_t *q)
{
    unsigned long length = (unsigned long)(esp_timer_get_time()) - ac_rx_last_falling_edge_time;
    ac_rx_last_falling_edge_time = (unsigned long)(esp_timer_get_time());
...
static void IRAM_ATTR jhs_panel_rx_isr(QueueHandle_t *q)
{
...
static void jhs_recv_task_func(void *arg)
{
    ...
    config_ptr->ac_rx_pin->attach_interrupt(jhs_ac_rx_isr, &ac_rx_queue, esphome::gpio::INTERRUPT_FALLING_EDGE);
